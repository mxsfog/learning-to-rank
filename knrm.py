...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> nn.ModuleList:
        kernels = nn.ModuleList()
        mu_list = self._generate_mu_list(self.kernel_num)
        for mu in mu_list[:-1]:
            kernels.append(GaussianKernel(mu=mu, sigma=0.1))
        kernels.append(GaussianKernel(mu=1.0, sigma=self.exact_sigma))
        return kernels

    def _generate_mu_list(self, kernel_num: int) -> List[float]:
        if kernel_num == 1:
            return [1.0]
        step = 2.0 / (kernel_num - 1)
        return [-1 + step * i for i in range(kernel_num)]

    def _get_mlp(self) -> torch.nn.Sequential:
        layers = []
        input_dim = self.kernel_num

        for output_dim in self.out_layers:
    ...
    def _get_matching_matrix(self, query: torch.Tensor, doc: torch.Tensor) -> torch.FloatTensor:
        ...
        doc_norm = doc / doc.norm(dim=1, keepdim=True)
        return torch.matmul(query_norm, doc_norm.transpose(-1, -2))
...
class RankingDataset(torch.utils.data.Dataset):
    ...
    def _convert_text_idx_to_token_idxs(self, idx) -> List[int]:
        ...
        return token_idxs[:self.max_len] + [0] * (self.max_len - len(token_idxs))
...
class TrainTripletsDataset(RankingDataset):
    def __getitem__(self, idx: int):
        query_id, pos_doc_id, neg_doc_id, label = self.index_pairs_or_triplets[idx]
        query_tokens = self._convert_text_idx_to_token_idxs(query_id)
        pos_doc_tokens = self._convert_text_idx_to_token_idxs(pos_doc_id)
        neg_doc_tokens = self._convert_text_idx_to_token_idxs(neg_doc_id)

        return {
            'query': torch.tensor(query_tokens, dtype=torch.long),
            'pos_doc': torch.tensor(pos_doc_tokens, dtype=torch.long),
            'neg_doc': torch.tensor(neg_doc_tokens, dtype=torch.long),
...
class ValPairsDataset(RankingDataset):
    def __getitem__(self, idx: int):
        ...
        query_tokens = self._convert_text_idx_to_token_idxs(query_id)
        doc_tokens = self._convert_text_idx_to_token_idxs(doc_id)
...
class Solution:
    ...
    def simple_preproc(self, inp_str: str) -> List[str]:
        # Очищаем строку от пунктуации и приводим к нижнему регистру
    ...
    def _filter_rare_words(self, vocab: Dict[str, int], min_occurancies: int) -> Dict[str, int]:
        # Фильтруем слова, которые встречаются реже, чем min_occurancies
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        # Формируем список всех текстов из DataFrame
        all_texts = []
        for df in list_of_df:
            all_texts.extend(df['question1'].tolist())
            all_texts.extend(df['question2'].tolist())

        # Применяем препроцессинг ко всем текстам и собираем все токены
        all_tokens = []
        for text in all_texts:
            all_tokens.extend(self.simple_preproc(text))

        # Считаем частоту каждого токена
        token_counts = Counter(all_tokens)

        # Фильтруем редкие слова
        filtered_vocab = self._filter_rare_words(token_counts, min_occurancies)

        # Возвращаем список токенов, которые проходят фильтр по частоте встречаемости
        return list(filtered_vocab.keys())
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        # Считываем эмбеддинги из файла
        glove_embeddings = self._read_glove_embeddings(file_path)

        # Инициализируем словарь и список неизвестных слов
        vocab = {'PAD': 0, 'OOV': 1}
        unk_words = []
        ...
        # Инициализируем матрицу эмбеддингов, включая PAD и OOV
        np.random.seed(random_seed)
        emb_matrix = np.zeros((len(inner_keys) + 2, embedding_dim))
        emb_matrix[1] = np.random.uniform(-rand_uni_bound, rand_uni_bound, embedding_dim)

        # Заполняем матрицу эмбеддингов
        for idx, word in enumerate(inner_keys, start=2):
            if word in glove_embeddings:
                emb_matrix[idx] = np.array(glove_embeddings[word], dtype=float)
                vocab[word] = idx
            else:
                emb_matrix[idx] = np.random.uniform(-rand_uni_bound, rand_uni_bound, embedding_dim)
                vocab[word] = idx
    ...
    def sample_data_for_train_iter(self, inp_df: pd.DataFrame, seed: int) -> List[List[Union[str, float]]]:
        random.seed(seed)
        queries = inp_df['query_id'].unique()
        triplets = []

        for query in queries:
            relevant_docs = inp_df[(inp_df['query_id'] == query) & (inp_df['relevance'] > 0)]
            irrelevant_docs = inp_df[(inp_df['query_id'] == query) & (inp_df['relevance'] == 0)]

            if len(relevant_docs) == 0 or len(irrelevant_docs) == 0:
                continue

            for rel_doc in relevant_docs['doc_id']:
                irrel_doc = irrelevant_docs.sample(1)['doc_id'].values[0]
                triplets.append([query, rel_doc, irrel_doc, 1.0])
    ...
    def ndcg_k(self, ys_true: np.array, ys_pred: np.array, ndcg_top_k: int = 10) -> float:
        order = np.argsort(ys_pred)[::-1]
        ys_true = ys_true[order]
        dcg = np.sum((2 ** ys_true - 1) / np.log2(np.arange(1, len(ys_true) + 1) + 1))
        ideal_dcg = np.sum((2 ** sorted(ys_true, reverse=True) - 1) / np.log2(np.arange(1, len(ys_true) + 1) + 1))
    ...
    def train(self, n_epochs: int):
        ...
        for epoch in range(n_epochs):
            self.model.train()
            triplets = self.sample_data_for_train_iter(self.train_df, seed=epoch)
            train_dataset = TrainTripletsDataset(triplets, self.idx_to_text_mapping, self.vocab, self.oov_val,
                                                 self.simple_preproc, max_len=30)
            train_loader = DataLoader(train_dataset, batch_size=1024, shuffle=True, collate_fn=self.collate_fn)

            for batch in train_loader:
                query = batch['query'].to(self.device)
                pos_doc = batch['pos_doc'].to(self.device)
                neg_doc = batch['neg_doc'].to(self.device)

                opt.zero_grad()
                pos_scores = self.model(query, pos_doc)
                neg_scores = self.model(query, neg_doc)
                loss = criterion(pos_scores, torch.ones_like(pos_scores)) + criterion(neg_scores,
                                                                                      torch.zeros_like(neg_scores))
                loss.backward()
                opt.step()

            if (epoch + 1) % self.change_train_loader_ep == 0:
                self.sample_data_for_train_iter(self.train_df, seed=epoch)

            ndcg = self.valid()
            print(f'Epoch {epoch + 1}/{n_epochs}, NDCG: {ndcg:.4f}')
